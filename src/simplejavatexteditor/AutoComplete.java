...
/**
 * <h1>Auto complete functionality multiple programming languages, including
 * brackets and parentheses</h1>
 *
 * <p>
 * An ArrayList is created for the keywords and the brackets. Logic for setting
 * the content of the ArrayList is found in UI.java. If the word currently being
 * typed matches a word in the list, a Runnable inner class is implemented to
...
public class AutoComplete implements DocumentListener {
	...
	private List<String> brackets = new ArrayList<>();
	private List<String> bracketCompletions = new ArrayList<>();
	private List<String> words = new ArrayList<>();
	private CodeCompletionMode mode = CodeCompletionMode.INSERT;
	private boolean isKeyword;
	private int pos;
	private String content;

	private enum CodeCompletionMode {
		INSERT, COMPLETION
	};


	public AutoComplete(UI ui, List<String> words) {
		this.ui = ui;
	...
	/**
	 * A character has been typed into the document. This method performs the
	 * primary check to find a keyword completion.
	 *
	 * @param event
	 */
	@Override
	public void insertUpdate(DocumentEvent event) {
		...
		if (n < 0 && -n < words.size()) {
			...
			if (match.startsWith(prefix)) {
				...
				mode = CodeCompletionMode.INSERT;
			}
		}
	}
	...
	private void initEnterKeyHandler() {
	...
	private void initKeywords(List<String> words) {
		this.words = words;
		supportedKeywords = new SupportedKeywords();
		brackets = supportedKeywords.getBrackets();
		bracketCompletions = supportedKeywords.getBracketCompletions();
		Collections.sort(this.words);
	}
	
	/**
	 * Handles the auto complete suggestion generated when the user is typing a word
	...
	private class CompletionTask implements Runnable {
		...
		public void run() {
			...
			mode = CodeCompletionMode.COMPLETION;
			if (!isKeyword) {
	...
	private class CommitAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (mode == CodeCompletionMode.COMPLETION) {
